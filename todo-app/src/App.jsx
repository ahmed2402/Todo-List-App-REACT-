...
function App() {
  ...
  return (
    <>
      ...
      <h1 className='font-bold text-center text-3xl'>Listify - Simplify your tasks, amplify your life</h1>
         <div className="addtodo my-5 flex flex-col gap-4">
        <h2 className='text-2xl font-bold'>Create a Task</h2> 
        <div className="flex">
        <input onChange={handleChange} value={todo} type="text" className=' font w-full rounded-full px-5 py-1' />
      ...
      <input className='my-4' id='show' onChange={toggleFinished} type="checkbox" checked={showFinished} /> 
         <label className=' font mx-2' htmlFor="show">Show Finished</label> 
         <div className='h-[1px] bg-black opacity-15 w-[90%] mx-auto my-2'></div>
     
                <h2 className='text-2xl font-bold'>Your Tasks</h2>
                <div className="todos">
                  {todos.length === 0 && <div className='m-5 text-xl font-bold'>All clear! Add a new task to stay productive.</div>}
                  {todos.map(item =>{

         return (showFinished || !item.iscompleted) && <div key={item.id} className={"font text-xl todo flex my-3 justify-between "}>
